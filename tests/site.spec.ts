...
test('event content and navigation are honest and complete', async ({ page }) => {
  ...
  await expect(sponsors.getByRole('heading', { name: 'Patrons of the Odyssey.' })).toBeAttached()
  await expect(sponsors.getByText('Seeking patrons', { exact: true })).toBeAttached()
  await expect(sponsors.getByRole('link', { name: 'Become a patron' })).toHaveAttribute('href', /^mailto:sponsor@hackuta\.org/)
